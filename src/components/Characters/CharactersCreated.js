import React, { useContext } from "react";
import CharacterContext from "../../context/Character/CharacterContext";

function CharactersCreated() {
  const ctx = useContext(CharacterContext);
  const { charactersCreatedFilter } = ctx;

  
  return (
    <>
      <div className="bg-gray-700 pt-6 pr-6 pl-6">
        <ul className="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 flex-row">
          {charactersCreatedFilter.map((character,i) => {
            return (
              <li className="col-span-1 flex flex-col text-center bg-white rounded-lg shadow divide-y divide-gray-200 ">
                <div class="flex-4 flex flex-col p-8 pl-4">
                  <img key={i}
                    className="w-32 h-32 flex-shrink-0 mx-auto rounded-full"
                    src={character.image}
                    alt="character"
                  />
                  <h3 key={i} className="mt-6 text-gray-900 text-sm font-medium">
                    {character.name}
                  </h3>
                  <dl className="mt-1 flex-grow flex flex-col justify-between">
                    <dt className="sr-only">Title</dt>
                    <dd key={i} className="text-gray-500 text-sm">
                      {character.location}
                    </dd>

                    <dt className="sr-only">Role</dt>
                    <dd className="mt-3">
                      <span key={i} className="px-2 py-1 text-green-800 text-xs font-medium bg-green-100 rounded-full">
                        {character.gender}
                      </span>
                    </dd>
                  </dl>
                </div>
                <div></div>
              </li>
            );
          })}
        </ul>
      </div>
    </>
  );
}

export default CharactersCreated;
